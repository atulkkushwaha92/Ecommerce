...
import {SHOPIT_PRODUCT_INFO_COOKIE, CART_TOTAL_COOKIE, AUTH_DETAILS_COOKIE} from '../constants/cookies'
...
export const sendPaymentToken = (token) => async dispatch => {
	...
	if (paymentResponse.payment_failed) {
		history.push(`/checkout/cancel-payment/${token.data.order_id}`)
	} else {
		history.push(`/checkout/success-payment/${token.data.order_id}`)
		Cookies.remove(CART_TOTAL_COOKIE)
		Cookies.remove(SHOPIT_PRODUCT_INFO_COOKIE)
	}
